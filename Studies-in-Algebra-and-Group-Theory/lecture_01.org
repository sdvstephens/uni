#+title: Studies-in-Algebra-and-Group-Theory - Lecture 1: Set Theory
...
Returning to our discussion of relations and partitions we can have this thing called an /induced relation/. Let \( R\coloneqq X / \mfC  \), we say that if \( x,y \in  \mfC \) and \(xX / \mfC y\), then \( X / \mfC \) is the relation /induced/ by the partition \( \mfC \).

#+NAME: Relationship between equivalence relations, classes, and set partitions
#+BEGIN_prop
...
\[ f : A \injto Y.\]
A special case of injectivity comes when we inject \( X \injto X \), relation-wise this is equality in \( X \), language-wise, we call this the */identity map/* on \( X \).

#+NAME: Restriction and extensions
#+BEGIN_prop
Consider a function \(f:Y\to Z \) and a set \( X  \subset  Y \). We say that there is a */natural way/* of constructing a new function \( g : X \to  Z \); let \(  g(x)=f(x), \forall x \in  X \), we then say \( g \) is a */restriction/* of \( f \) to \( X \) and \( f \) is the */extension/* of \(  g \) to \( Y \). We denote this as \( g=f|X  \) where we can express the definition as \((f|X)(x)=f(x) \forall x \in X \) and \(  \ran(f|X)=f(x_X) \).
#+END_prop
...
#+NAME: Projection
#+BEGIN_dfn
\( f \) (also \( \pr \) is a */projection/* from \( X \times Y \) onto \( X \) if if \( f(x,y)=x \); similarly, if \( R=X \times  Y \), then instead of being a projection of \( R \) onto \( X \), it is now the range of the projection \( f \) (\( \ran (\pr ) \)).
#+END_dfn

#+NAME: Bijection
#+BEGIN_dfn
Expounding on our notion of inclusion mappings and surjections from earlier, */bijectivity/*---one-to-one correspondence-ness---is a function that is both injective and surjective.
#+END_dfn
 ...
 #+BEGIN_dfn
If \( A \subset X \) the characteristic function of \( A \) is the function\(\mcX : X \to 2 \) given by
 \[ \mcX(x)=\begin{cases}
      1 & \text{if } x \in  A \\
      0 & \text{if } x\in X-A
  \end{cases} \],
...
a. \( X = \emptyset \), vacuously fulfilled (it matters not what \( R \) is);
b. \( R \coloneqq \left\{ (x,x) \mid x \in  X \right\} \), so every equivalence class is a singleton.
#+END_sol
...
#+NAME:
#+BEGIN_ex
Let us now consider an uncountably infinite indexing set it a plane \( I = [-1,1], A_i=\{i\}\times [0,1] \subset  \mathbb{R}^2   \). Notice this is going to be a subset of ordered pairs, i.e., \( \{(i,y) \mid  0 \le y \le 1\}   \). Geometrically, the union over all \( i\in I \) of \( A_i \) gives us a rectangle in \( \mathbb{R}^2 \) defined by \( [-1,1]\times [0,1] \); similarly the intersection of these sets, notice, are intersections of finite parallel lines, thus there exists no element in common with these sets, thus it is \( \emptyset \).
#+END_ex

#+NAME: Associative law of unions
#+BEGIN_thm
...
#+BEGIN_note
Regarding notation---as we all too often are---\( \Img (f_A) \) is the same thing as \( f(A) \) when regarding images of subsets. I prefer the former notation as it feels a bit more natural to me.
#+END_note

#+NAME: Inverses and inverse images
#+BEGIN_dfn
Given a function \( f : X \to  Y \) we say \( f^{-1} \) is the */inverse/* of \( f \) given by \( f^{-1} : \mcP(Y) \to  \mcP(X) \) such that if \( B \subset  Y \), then
\[
...
#+BEGIN_note
We often use \( f^{-1} \) for another purpose: for some \( f:X \to Y  \), \( f^{-1} : \ran (f)\to X \implies f^{-1}(y)=x \iff f(x)=y\). This will be the most common use case for \( f^{-1} \) as the inverse function \( g : Y \to X \)
#+END_note
...
Let \( f \) be a function such that \( f : X \to A \) for each of the following,
  ...
  2. \( f : X \surjto   Y, \, B \subset Y \implies f(f^{-1}(B))=B\);
  3. \( A \subset X \implies A \subset  f^{-1}(f(A)) \);
  4. \( f : X \injto  Y , \, A \subset X  \implies f^{-1}(f(A))=A \);
