...
Then \(\pi : V \to W\) is \(G\)-equivariant (\(g\pi g^{-1} = \pi\)), and \(\pi|_W = \id\) (since \(g\pi_0(g^{-1}w) = g(g^{-1}w) = w\) for \(w \in W\), as \(g^{-1}w \in W\)). So \(\pi\) is a \(G\)-equivariant projection onto \(W\), and \(U = \Ker(\pi)\) is a \(G\)-invariant complement.
}
...
\subsection{Column Orthogonality}
...
\mprop{Column orthogonality relations}{Let \(C_1, \ldots, C_k\) be the conjugacy classes of \(G\), with representatives \(g_1, \ldots, g_k\) and sizes \(|C_1|, \ldots, |C_k|\). Then
\[\sum_{i=1}^k \chi_{V_i}(g_s) \, \overline{\chi_{V_i}(g_t)} = \frac{|G|}{|C_s|} \delta_{st}.\]%
}

\pf{Proof}{Consider the \(k \times k\) character table matrix \(X\) with entries \(X_{ij} = \chi_{V_i}(g_j)\). Row orthonormality states \(\frac{1}{|G|} \sum_j |C_j| \, X_{ij} \overline{X_{\ell j}} = \delta_{i\ell}\). In matrix form, \(X D \overline{X}^T = I\) where \(D = \diag(|C_1|/|G|, \ldots, |C_k|/|G|)\). Since \(X\) is square and invertible, \(X^{-1} = D \overline{X}^T\). Taking the transpose: \((X^T)^{-1} = \overline{X} D\), so \(X^T \overline{X} D = I\), giving \(X^T \overline{X} = D^{-1}\). Entry-by-entry, this is the column orthogonality relation.%
}

\nt{Column orthogonality provides an independent check on computed character tables. It also has a group-theoretic interpretation: two elements \(g, h \in G\) are conjugate if and only if \(\chi_V(g) = \chi_V(h)\) for every representation \(V\). Characters separate conjugacy classes.}
