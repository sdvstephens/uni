...
\pf{Proof}{Consider the \(k \times k\) character table matrix \(X\) with entries \(X_{ij} = \chi_{V_i}(g_j)\). Row orthonormality states \(\frac{1}{|G|} \sum_j |C_j| \, X_{ij} \overline{X_{\ell j}} = \delta_{i\ell}\). In matrix form, \(X D \overline{X}^T = |G| \cdot I\) where \(D = \diag(|C_1|/|G|, \ldots, |C_k|/|G|)\). Since \(X\) is square and invertible (the rows are linearly independent), this gives \(\overline{X}^T X D = |G| \cdot I\), which rearranges to the column relations.%
}
