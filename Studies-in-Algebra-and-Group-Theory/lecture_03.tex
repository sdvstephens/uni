...
\pagebreak
\ex{Quotient subgroups of \( S_{3} \)}{ Note that \( S_{3} \) is isomorphic to \( \mathbb{Z}/6 \), thus we may think of of the subgroups as such: 
  \[
    S_{3} \cong \mathbb{Z}/6 \cong \mathbb{Z}/3 \times  \mathbb{Z}/ 2
  \] 
  ...
  % Middle row - order 2 subgroups
  \node (a) at (-2,1) {$\langle \rho  \rangle$};
  \node (b) at (-0.7,1) {$\langle \mu \rangle$};
  \node (ab) at (0.7,1) {$\langle \rho  \mu \beta\rangle$};
...
A question naturally arises then: when is \(G/H\) a group? If we try to define \((aH)(bH) = abH\), we need this to be \emph{well-defined}, meaning if \(aH = a'H\) and \(bH = b'H\), then \(abH = a'b'H\). This does \emph{not} always work!
