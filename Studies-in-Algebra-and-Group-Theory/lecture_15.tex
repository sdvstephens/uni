...
\section{Dimension Formulas and Irreducibility}

\mprop{Dimension of induced representations}{\(\dim \operatorname{Ind}^G_H W = [G:H] \cdot \dim W\). This is immediate from the construction \(V = \bigoplus_{\sigma \in G/H} \sigma W\).%
}

\nt{An induced representation is almost never irreducible. For instance, \(\operatorname{Ind}^G_{\{e\}}({\CC}) = {\CC}[G]\) (the regular representation), which decomposes into all irreducibles of \(G\). Frobenius reciprocity gives a criterion: \(\operatorname{Ind}^G_H W\) is irreducible iff \(H(\chi_{\operatorname{Ind} W}, \chi_{\operatorname{Ind} W}) = 1\), which by Frobenius equals \(\langle \chi_{\operatorname{Ind} W}, \chi_{\operatorname{Ind} W} \rangle_G = \langle \chi_W, \chi_{\operatorname{Res} \operatorname{Ind} W} \rangle_H\). Analyzing \(\operatorname{Res}^G_H \operatorname{Ind}^G_H W\) leads to Mackey's formula, which we state without proof.}

\mprop{Mackey's irreducibility criterion}{Let \(W\) be an irreducible representation of \(H \leq G\). Then \(\operatorname{Ind}^G_H W\) is irreducible if and only if for every \(g \in G \setminus H\), the representations \(W\) and \(W^g\) of \(H \cap gHg^{-1}\) have no common irreducible constituent, where \(W^g\) denotes the conjugate representation \(W^g(h) = W(g^{-1}hg)\) restricted to \(H \cap gHg^{-1}\).%
}

\ex{Induction from normal subgroups}{If \(H \trianglelefteq G\) and \(W\) is an irreducible representation of \(H\), then \(\operatorname{Ind}^G_H W\) is irreducible iff for all \(g \in G \setminus H\), the conjugate \(W^g\) is not isomorphic to \(W\) as an \(H\)-representation. In other words, \(W\) is not fixed by any nontrivial element of \(G/H\) acting on the set of irreducible representations of \(H\) by conjugation.%
}

\ex{Inducing from index-2 subgroups}{Let \([G:H] = 2\) and \(W\) an irreducible representation of \(H\). Since \(H\) is normal of index 2, there is a single nontrivial coset representative \(g\). Then \(\operatorname{Ind}^G_H W\) is irreducible iff \(W \not\cong W^g\), and splits as a direct sum of two irreducibles (each of dimension \(\dim W\)) when \(W \cong W^g\).

For example, let \(G = S_3\) and \(H = A_3 \cong {\ZZ}/3\). The nontrivial irreducibles of \(A_3\) are the characters \(\chi_\omega : (123) \mapsto \omega\) and \(\chi_{\omega^2} : (123) \mapsto \omega^2\) (where \(\omega = e^{2\pi i/3}\)). Conjugation by \((12)\) swaps \(\chi_\omega \leftrightarrow \chi_{\omega^2}\), so \(\chi_\omega \not\cong \chi_\omega^g\) and \(\operatorname{Ind}^{S_3}_{A_3} \chi_\omega\) is irreducible. Since \(\dim = [S_3:A_3] \cdot 1 = 2\), this is the standard representation \(V\) of \(S_3\).%
}
...
\textbf{(3)} By Frobenius reciprocity, the multiplicity of an \(S_5\)-irreducible \(X\) in \(\operatorname{Ind}^{S_5}_{S_4}(V_4)\) equals \(\langle \chi_{V_4}, \chi_{\operatorname{Res}(X)} \rangle_{S_4}\). We restrict each \(S_5\)-irreducible to \(S_4\) (the subgroup fixing 5) by evaluating its character on elements of cycle types \(1^5, 2 \cdot 1^3, 2^2 \cdot 1, 3 \cdot 1^2, 4 \cdot 1\). The \(S_4\) inner product with \(\chi_{V_4} = (3, 1, -1, 0, -1)\) (class sizes 1, 6, 3, 8, 6) gives:
\begin{align*}
\langle \operatorname{Res}(U), \chi_{V_4} \rangle &= \tfrac{1}{24}(3 + 6 - 3 + 0 - 6) = 0, \\
\langle \operatorname{Res}(U'), \chi_{V_4} \rangle &= \tfrac{1}{24}(3 - 6 - 3 + 0 + 6) = 0, \\
\langle \operatorname{Res}(V), \chi_{V_4} \rangle &= \tfrac{1}{24}(12 + 12 + 0 + 0 + 0) = 1, \\
\langle \operatorname{Res}(V'), \chi_{V_4} \rangle &= \tfrac{1}{24}(12 - 12 + 0 + 0 + 0) = 0, \\
\langle \operatorname{Res}(\textstyle\bigwedge^2 V), \chi_{V_4} \rangle &= \tfrac{1}{24}(18 + 0 + 6 + 0 + 0) = 1, \\
\langle \operatorname{Res}(W), \chi_{V_4} \rangle &= \tfrac{1}{24}(15 + 6 - 3 + 0 + 6) = 1, \\
\langle \operatorname{Res}(W'), \chi_{V_4} \rangle &= \tfrac{1}{24}(15 - 6 - 3 + 0 - 6) = 0.
\end{align*}
So \(\operatorname{Ind}^{S_5}_{S_4}(V_4) = V \oplus \bigwedge^2 V \oplus W\) (dimensions \(4 + 6 + 5 = 15 = [S_5:S_4] \cdot 3\)).
