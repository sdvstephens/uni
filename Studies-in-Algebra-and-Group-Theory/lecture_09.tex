...
\[\langle T^*(v_1 + \lambda v_2), w \rangle = \langle v_1 + \lambda v_2, T(w) \rangle = \langle v_1, T(w) \rangle + \lambda \langle v_2, T(w) \rangle\]
\[= \langle T^*(v_1), w \rangle + \lambda \langle T^*(v_2), w \rangle = \langle T^*(v_1) + \lambda T^*(v_2), w \rangle.\]
...
\pf{Proof}{
  \textbf{Case 1: $x = y$.}
  If $x = 0$, any reflection fixes the origin, so any reflection works.
  If $x \neq 0$ (and $\dim V \geq 2$), choose any unit vector $v$ orthogonal to $x$
  and set $P(z) = z - 2\langle z, v \rangle v$.
  Then $P(x) = x - 2\langle x, v \rangle v = x = y$.

  \textbf{Case 2: $x \neq y$.}
  Let $v = \dfrac{x - y}{\|x - y\|}$ and define $P(z) = z - 2\langle z, v \rangle v$.

  Compute:
  \[
    P(x) = x - 2\langle x, v \rangle v
         = x - \frac{2\langle x,\, x - y \rangle}{\|x - y\|^2}(x - y).
  \]
  ...
  \[
    \langle x,\, x - y \rangle
      ...
      = \tfrac{1}{2}\bigl(\|x\|^2 + \|y\|^2 - 2\langle x, y \rangle\bigr)
      = \tfrac{1}{2}\|x - y\|^2.
  \]

  Substituting:
  \[
    P(x) = x - \frac{2 \cdot \tfrac{1}{2}\|x - y\|^2}{\|x - y\|^2}(x - y)
         = x - (x - y)
     = y. \] }
...
\begin{enumerate}[(i)]
    ...
    \item \(G(u, v) = B(Ju, v)\) for all \(u, v \in W\).
...
(iii) \(B(Ju, v) = \operatorname{Im} H(iu, v) = \operatorname{Im}(i \cdot H(u, v)) = \operatorname{Re} H(u, v) = G(u, v)\).

(iv) Suppose \(H\) is nondegenerate and \(G(u, v) = 0\) for all \(v\). Then \(B(Ju, v) = G(u, v) = 0\) for all \(v\), so \(B(w, v) = 0\) for all \(w, v\) (since \(J\) is surjective). Thus \(H(u, v) = G(u, v) + iB(u, v) = 0\) for all \(v\), giving \(u = 0\). The other implications are similar.
}
...
\textbf{(3)} We have $T(S) = \{w : \|T^{-1}(w)\| = 1\}$. Now
\[
...
where $Q = (T^{-1})^* T^{-1}$ is self-adjoint and positive-definite.
 ...
 By the spectral theorem, there exists an orthonormal eigenbasis \(\{u_1, \ldots, u_n\}\) with \(Q(u_i) = \mu_i u_i\), where \(\mu_i > 0\).
