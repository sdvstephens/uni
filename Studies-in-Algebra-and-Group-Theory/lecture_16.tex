...
Now \(\tau^2 : V \to V\) is \(\CC\)-linear and \(G\)-equivariant, so by Schur's lemma, \(\tau^2 = \lambda \cdot \id\) for some \(\lambda \in \CC\). We show \(\lambda \in \RR_{> 0}\):
\[H(\tau^2(v), v) = B(\tau(v), v) = B(v, \tau(v)) = H(\tau(v), \tau(v)) \geq 0\]
where the second equality uses symmetry of \(B\). So \(\lambda \geq 0\), and \(\lambda > 0\) since \(\tau \neq 0\). Replacing \(H\) by \(\lambda^{-1/2} H\) (or equivalently \(\tau\) by \(\lambda^{-1/2} \tau\)), we arrange \(\tau^2 = \id\). By the antilinear involution criterion, \(V\) is real.
}
