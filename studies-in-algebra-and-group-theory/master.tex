...
\begin{enumerate}
  \item \( \dom f = X  \);
  \item if \( (x,y)\in f \) and \( (x,z) \in f \), then \( y=z \) (uniqueness of \( y \) for each \( x \)); additionally, \( f(x)=y \) is conventional notation (insteaf of \( (x,y) \in f \) or \( xfy \));
...
\end{enumerate}}
\sol \pf{Proof}{\begin{enumerate}[a)]
  \item 
\end{enumerate}}
